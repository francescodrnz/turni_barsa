...
def read_pdf_tables(file_path):
    """Read tables from PDF file using pdfplumber and print all content."""
    try:
        with pdfplumber.open(file_path) as pdf:
            print("\nCONTENUTO COMPLETO DEL PDF:")
            print("=" * 80)
            ...
            for page_num, page in enumerate(pdf.pages, 1):
                ...
                # Estrai e stampa il testo completo della pagina
                print("\nTesto della pagina:")
                print(page.extract_text())
                print("\nTabelle trovate:")
                
                # Estrai e stampa le tabelle
                tables = page.extract_tables()
                for table_num, table in enumerate(tables, 1):
                    print(f"\nTabella {table_num}:")
                    for row in table:
                        # Sostituisci None con spazi vuoti per la stampa
...
def get_bathroom_cleaning_time(location, time_slot):
    """Determine bathroom cleaning time based on location and time slot."""
    if "giardini del castello" in location.lower():
        if time_slot and "14:00-21:00" in time_slot:
            return "ore 19"
        elif time_slot and "08:00-14:00" in time_slot:
            return "ore 12"
    return ""

def print_shifts_to_console(shifts):
    """Print extracted shifts to console in a formatted way."""
    print("\nTURNI ESTRATTI DAL PDF:")
    print("-" * 100)
    print(f"{'GIORNO':<15} {'NUMERO':<8} {'LUOGO':<35} {'ORARIO':<15} {'PULIZIA BAGNI'}")
    print("-" * 100)
    
    for day, day_number, location, time in shifts:
        pulizia = get_bathroom_cleaning_time(location, time)
        print(f"{day:<15} {day_number:<8} {location:<35} {time:<15} {pulizia}")
    print("-" * 100)
...
def write_shifts_to_pdf(shifts, input_filename, surname):
    ...
    if match:
        output_filename = f"Turni {surname} " + match.group(0)[0:]
    else:
        output_filename = "TURNI.pdf"
    ...
    # Larghezze colonne modificate per aggiungere la nuova colonna
    col_widths = {
        'giorno': 30,
        'numero': 20,
        'luogo': 70,
    ...
    pdf.cell(col_widths['giorno'], 10, 'Giorno', border='LTB', align='R', fill=True)
    pdf.cell(col_widths['numero'], 10, ' ', border='RTB', align='C', fill=True)
    pdf.cell(col_widths['luogo'], 10, 'Luogo', border=1, align='C', fill=True)
    pdf.cell(col_widths['orario'], 10, 'Orario', border=1, align='C', fill=True)
    pdf.cell(col_widths['pulizia'], 10, 'Pulizia Bagni', border=1, align='C', fill=True)
    pdf.ln()

    # Aggiungi i turni
    for day, day_number, location, time in shifts:
        pdf.cell(col_widths['giorno'], 10, day, border='LTB', align='R')  # Giorno
        pdf.cell(col_widths['numero'], 10, day_number, border='RTB', align='L')  # Numero
        pdf.cell(col_widths['luogo'], 10, location, border=1, align='C')  # Luogo
        pdf.cell(col_widths['orario'], 10, time, border=1, align='C')  # Orario
        
        # Aggiungi la colonna pulizia bagni con l'orario appropriato
        pulizia = get_bathroom_cleaning_time(location, time)
        pdf.cell(col_widths['pulizia'], 10, pulizia, border=1, align='C')
        
        pdf.ln()

    # Salva il PDF
    pdf.output(output_filename)
    print(f"\nFile '{output_filename}' creato con successo!")
...
def main():
    ...
    # Stampa i turni sulla console
    print_shifts_to_console(shifts_sorted)
